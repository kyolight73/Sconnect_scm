...
@section('content')
    <div class="container">
        {!! App\Utils::createBreadcrumb(['Media', 'Kênh phân phối']) !!}
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="/channel/save">@csrf
                            ...
                            <input type="hidden" name="sl_channel_type" value="{{ $sl_channel_type }}"
                                id="sl_channel_type" />
                            ...
                            <div>
                                <input type="hidden" name="platform_id"
                                    value="{{ old('platform_id', !empty($edit_channel) ? $edit_channel->platform_id : '0') }}"
                                    id="platform_id" />
                                <input type="hidden" name="selected_platform_name" id="selected_platform_name_hidden"
                                    value="{{ $selected_platform_name }}" />

                                <div data-toggle="dropdown" aria-expanded="true" class="form-control cursor-hand">
                                    &nbsp;<span id="selected_platform_name">{!! $selected_platform_name !!}</span><span
                                        style="display:inline-block; float: right; margin-top: 10px; margin-right: -8px;"
    						  ...
    						  transform: translate3d(0px, -165px, 0px); top: 0px; left: 0px; will-change: transform; background-color: #eee!important">
                                        <div class="dropdown-item cursor-hand" style="width: 100%"
                                            onclick="selectPlatform(0, '&nbsp; -- Tất cả nền tảng -- &nbsp;', '')"> &nbsp;
                                            -- Tất cả nền tảng -- &nbsp; </div>
                                        @foreach ($platform_list as $platform)
                                            <div class="dropdown-item cursor-hand" style="width: 100%"
                                                onclick="selectPlatform({{ $platform->id }}, '{{ htmlspecialchars($platform->name) }}', '{{ $platform->picture }}')">
                                                <img src="{{ $platform->picture }}" width="40" height="40" /> &nbsp;
                                                {{ $platform->name }}
                                            </div>
                                        @endforeach
                                    </div>
                            ...
                            <div>
                                <select id="channel_type_id" name="channel_type_id" class="form-control">
                                    ...
                                    @foreach ($channel_type_list as $channel_type)
                                        <option
                                            value="{{ $channel_type->id }}"{{ $channel_type->id == $channel_type_id ? ' selected' : '' }}>
                                            {{ $channel_type->name }}</option>
                                    @endforeach
                                </select>
                                <div class="arrow-down" style="float: right; margin-top: -21px; margin-right: 4px;"></div>
                            </div>
                            ...
                            <div>
                                <select id="staff_manager_id" name="staff_manager_id" class="form-control">
                                    ...
                                    @foreach ($staff_list as $staff)
                                        <option
                                            value="{{ $staff->id }}"{{ $staff->id == $staff_manager_id ? ' selected' : '' }}>
                                            {{ $staff->name }}</option>
                                    @endforeach
                                </select>
                                <div class="arrow-down" style="float: right; margin-top: -21px; margin-right: 4px;"></div>
                            ...
                            <input id="name" type="text"
                                class="form-control margin-top @error('name') is-invalid @enderror" name="name"
                                value="{{ old('name', !empty($edit_channel) ? $edit_channel->name : 'NONAME') }}" required
                                autofocus placeholder="Tên kênh">
                            <input id="url" type="text"
                                class="form-control margin-top @error('url') is-invalid @enderror" name="url"
                                value="{{ old('url', !empty($edit_channel) ? $edit_channel->url : '') }}" required
                                placeholder="Đường link của kênh">
                            <input id="schedule" type="text"
                                class="form-control margin-top @error('schedule') is-invalid @enderror" name="schedule"
                                value="{{ old('schedule', !empty($edit_channel) ? $edit_channel->schedule : '') }}"
                            ...
                            <div class="justify-content-between"
                                ...
                                @if (is_numeric($id) && $id > 0)
                                    <button type="button" class="btn btn-default" id="btn-cancel">Huỷ sửa</button>
                                @endif
                            </div>
                        </form>
                    </div>
            ...
            <div class="col-md-8">
                    ...
                    <div class="card-body">
                        <form method="get" action="/channel">
                            <div class="bold">Traffic tổng các kênh - Tháng
                                ...
                                <button type="submit" class="btn btn-sm btn-success">&nbsp; &nbsp; Lọc &nbsp;
                                    &nbsp;</button>
                            </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5>Danh sách kênh video<div
                                style="float: right; font-weight: normal; font-size: 13px; cursor: pointer; margin-top: 5px;">
                                <i class="fas fa-poll"></i> Báo cáo
                            </div>
                        </h5>
                        <form method="GET" action="/channel" name="frm_select_qtk" id="frm_select_qtk">
                            @php
                                $sl_platform = request()->input('sl_platform');
                                if (empty($sl_platform)) {
                                    $sl_platform = 0;
                                }
                            ...
                            @endphp
                            <div>
                                <span style="white-space: nowrap; padding-top: 10px; display: inline-block;">Nền tảng:
                                    <select name="sl_platform" id="sl_platform" style="padding: 4px 15px;"
                                        onchange="frm_select_qtk.submit()">
                                        <option value="0">-- Tất cả nền tảng --</option>
                                        @foreach ($platform_list as $platform)
                                            <option
                                                value="{{ $platform->id }}"{{ $sl_platform == $platform->id ? ' selected' : '' }}>
                                                {{ $platform->name }}</option>
                                        @endforeach
                                    </select>
                                </span> &nbsp; &nbsp; &nbsp;
                                <span style="white-space: nowrap; padding-top: 10px; display: inline-block;">Chủ đề:
                                    <select name="sl_topic" id="sl_topic" style="padding: 4px 15px;"
                                        onchange="frm_select_qtk.submit()">
                                        <option value="0">-- Tất cả chủ đề --</option>
                                        {!! $html_sl_topic !!}
                                    </select></span>
                            ...
                            <div style="padding-bottom: 5px">
                                <span style="white-space: nowrap; padding-top: 10px; display: inline-block;">Loại kênh:
                                    <select id="sl_channel_type" name="sl_channel_type" style="padding: 4px 15px;"
                                        onchange="frm_select_qtk.submit()">
                                        <option value="0">-- Chọn loại kênh --</option>
                                        @foreach ($channel_type_list as $channel_type)
                                            <option
                                                value="{{ $channel_type->id }}"{{ $sl_channel_type == $channel_type->id ? ' selected' : '' }}>
                                                {{ $channel_type->name }}</option>
                                        @endforeach
                                    </select>
                                </span> &nbsp; &nbsp; &nbsp;
                                <span style="white-space: nowrap; padding-top: 10px; display: inline-block;">Quản trị kênh:
                                    <select id="sl_manager" name="sl_manager" style="padding: 4px 15px;"
                                        onchange="frm_select_qtk.submit()">
                                        <option value="0">-- Chọn quản trị viên --</option>
                                        @foreach ($staff_list as $staff)
                                            <option
                                                value="{{ $staff->id }}"{{ $sl_manager == $staff->id ? ' selected' : '' }}>
                                                {{ $staff->name }}</option>
                                        @endforeach
                                    </select>
                                </span>
                            ...
                            @if (!empty($channel_list) && count($channel_list) > 0)
                                <tbody>
                                    @foreach ($channel_list as $channel)
                                        ...
                                        <tr>
                                            <td align="center">
                                                <div><img src="{{ $channel->thumbnail }}" alt="Thumb" width="50"
                                                        height="50"
                                                ...
                                                <div style="padding-top: 10px; font-size: 12px;"
                                                    class="{{ $cls_name[$channel->status] }}">
                                                    <i class="fas fa-circle {{ $cls_name[$channel->status] }}"></i>
                                            ...
                                            <td>
                                                <div>
                                                    <a
                                                        href="/video?channel={{ $channel->id }}"><strong>{{ $channel->name }}</strong></a>
                                                    &nbsp;
                                                    <span class="cursor-hand"
                                                        onclick="updateContentOfChannel({{ $channel->id }})"><i
                                                            class="fas fa-sync-alt"></i></span>
                                                </div>
                                                <div>
                                                    <a href="{{ $channel->url }}" target="_blank">
                                                        <span
                                                            style="max-width:350px; display:-webkit-box; -webkit-line-clamp: 1;-webkit-box-orient: vertical; overflow: hidden; color: #888888; font-size: 90%; text-overflow:ellipsis;">{{ $channel->url }}</span>
                                                    </a>
                                                </div>
                                                <div>
                                                ...
                                                <div style="color: #888888; font-size: 90%; ">
                                                    ...
                                                    <span class="nowrap">&bull; Nền tảng: <span
                                                            class="text-color1">{{ $platform->name }}</span></span>
                                                    <span class="nowrap">&bull; Chủ đề: <span
                                                            class="text-color1">{{ $topic->name }}</span> </span>
                                                    <span class="nowrap">&bull; Loại kênh: <span
                                                            class="text-color1">{{ !empty($channel_type) ? $channel_type->name : '' }}</span></span>
                                                    <span class="nowrap">&bull; Quản trị kênh: @if (empty($manager))
                                                            <a
                                                                href="/channel?sl_platform={{ $sl_platform }}&sl_topic={{ $sl_topic }}&sl_channel_type={{ $sl_channel_type }}&sl_manager={{ $sl_manager }}&channel={{ $channel->id }}"><i
                                                                    style="color: #ff5648">[Chưa có]</i></a>
                                                        @else
                                                            <span class="text-color1">{{ $manager->name }}</span>
                                                        @endif
                                                    </span>
                                                </div>
                                            </td>
                                            ...
                                            <td
                                                style="text-align: center; vertical-align: middle; padding: 2px!important;">
                                                <div>
                                                    <a href="javascript: void(0)" title="Xem chi tiết"><i
                                                            class="far fa-list-alt cursor-hand"></i></a> &nbsp;
                                                    <a href="/channel?sl_platform={{ $sl_platform }}&sl_topic={{ $sl_topic }}&sl_channel_type={{ $sl_channel_type }}&sl_manager={{ $sl_manager }}&channel={{ $channel->id }}"
                                                        title="Sửa thông tin"><i class="far fa-edit ic24"></i></a>
                                                    <a href="/channel/delete?sl_platform={{ $sl_platform }}&sl_topic={{ $sl_topic }}&sl_channel_type={{ $sl_channel_type }}&sl_manager={{ $sl_manager }}&channel={{ $channel->id }}"
                                                        ...
                                                        <i class="far fa-trash-alt ic24 cursor-hand" title="Xoá kênh"
                                                            style="color: #ff5648!important"></i>
                                                    </a>
                                                </div>
                                            </td>
                                        </tr>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $('#btn-cancel').on('click', function() {
                location.href = '/channel?platform=' + $('#platform_id').val() + '&topic=' + $('#topic_id')
                    .val();
            });
            ...
            @endif
            {!! !empty(old('topic_id'))
                ? '$("select#topic_id option[value=\'' . old('topic_id') . '\']").attr("selected",true);'
                : '' !!}
            {!! !empty(old('channel_type_id'))
                ? '$("select#channel_type_id option[value=\'' . old('channel_type_id') . '\']").attr("selected",true);'
                : '' !!}
            {!! !empty(old('staff_manager_id'))
                ? '$("select#staff_manager_id option[value=\'' . old('staff_manager_id') . '\']").attr("selected",true);'
                : '' !!}
        ...
        function selectPlatform(platform_id, name, image) {
            ...
            var selected_platform_name = '<img style="margin-top: -4px;" src="' + image +
