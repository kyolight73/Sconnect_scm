                        ...
                        <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link{{ $tab == 1 ? ' active' : '' }}" id="custom-tabs-four-1-tab"
                                   href="?tab=1&video_id={{$video_id}}&d={{$from_detail}}">Đang duyệt
                                    ({{ $tab_item_count[1] }})</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link{{ $tab == 2 ? ' active' : '' }}" id="custom-tabs-four-2-tab"
                                   href="?tab=2&video_id={{$video_id}}&d={{$from_detail}}">Đang chạy
                                    ({{ $tab_item_count[2] }})</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link{{ $tab == 3 ? ' active' : '' }}" id="custom-tabs-four-3-tab"
                                   href="?tab=3&video_id={{$video_id}}&d={{$from_detail}}">Tạm dừng
                                    ({{ $tab_item_count[3] }})</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link{{ $tab == 4 ? ' active' : '' }}" id="custom-tabs-four-4-tab"
                                   href="?tab=4&video_id={{$video_id}}&d={{$from_detail}}">Đã kết thúc
                                    ({{ $tab_item_count[4] }})</a>
                            </li>
                        </ul>
                            ...
                            <div class="form-group"
                                ...
                                <select class="form-control select2" style="width: 400px;"
                                    ...
                                    @foreach($video_list as $video)
                                        <option
                                            value="{{ $video->id }}"{{ $video_id == $video->id ? ' selected' : '' }}>{{ $video->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endif
                        <div class="tab-content" id="custom-tabs-four-tabContent">
                            <div class="tab-pane table-responsive p-0 fade show active" id="custom-tabs-four-new"
                                 role="tabpanel" aria-labelledby="custom-tabs-four-new-tab">
                                    ...
                                    @foreach($ticket_list as $idle_ticket)
                                        <tr align="center">
                                            <td align="left">
                                                ...
                                                <div style="font-size: 85%; font-weight: bold;">Campaign
                                                    ID: {{ $idle_ticket->campaign_id }}</div>
                                            </td>
                                            <td>{{ number_format($idle_ticket->budget) }}đ</td>
                                            <td align="left">{{ $idle_ticket->start_date->format('H:i d/m/Y') }} <br/><i
                                                    class="fas fa-arrow-right"></i> {{ $idle_ticket->end_date->format('H:i d/m/Y') }}
                                            </td>
                                            ...
                                            <td>
                                                <i class="cursor-hand far fa-solid fa-tag"
                                                   ...
                                                   title="{{ $idle_ticket->keyword }}"></i>
                                            </td>
                                            <td>
                                                <div style="position: relative;" data-toggle="modal"
                                                     data-target="#modal-ticket-comment"
                                                     data-ticketid="{{ $idle_ticket->id }}">
                                                    <i class="cursor-hand far fa-comment-alt"></i>@php
                                                        $unread_count = $idle_ticket->total_unread_comment;
                                                    @endphp
                                                    @if ($unread_count > 0)
                                                        <span id="badge-{{ $idle_ticket->id }}"
										...
										{{ $idle_ticket->total_unread_comment }}
                                            ...
                                            <td>{{ \App\Models\Ticket::WORKFLOW[$idle_ticket->workflow_position] }}</td>
                                            <td>
                                                <i class="cursor-hand fas fa-edit" data-toggle="modal"
                                                   data-target="#modal-promote-ticket"
                                                   data-ticketid="{{ $idle_ticket->id }}" data-action="edit"></i> &nbsp;
                                                @if ($idle_ticket->workflow_position < 2)
                                                    <i class="cursor-hand fas fa-trash-alt"
                                                       onclick="delete_ticket({{ $idle_ticket->id }}, '{{ $idle_ticket->title }}')"></i>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="row">
                                <div class="col col-3">
                                    <div><img src="{{ $selected_video->thumbnail }}" style="width: 100%; height:auto;"/>
                                    </div>
                                    <div style="padding: 5px 0px;"><a href="{{ $selected_video->url }}" target="_blank"><strong>{{ $selected_video->name }}</strong></a>
                                    </div>
                                    <div class="justify-content-between text-grey"
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
							...
							<span class="cursor-hand btn-round" data-toggle="modal"
                                  data-target="#modal-import-ads-report">
